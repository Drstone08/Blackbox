...
function Page4() {
  ...
  const hnt= import.meta.env.VITE_HINT4;

  const handleSubmit = async () => {
    if (ans2.toLowerCase() === import.meta.env.VITE_ANS4) {
      const now = new Date();
  ...
  const handleHint = () => {
    const confirmHint = window.confirm(
      "Your score will be deducted by 3. Do you want to proceed? Click 'OK' for Yes and 'Cancel' for No."
    );
    if (confirmHint) {
      setHintused(true);
      setHint(true);
